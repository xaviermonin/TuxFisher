...
bool gestionNiveau_TuxFisher::chargerFichier(QIODevice &fichierniveau)
{
	...
	{
		erreur = QObject::tr("Le fichier de la partie n'a pas été ouvert.");
		return false;
	}
	int nbcarte;
	char caract;
	...
	if (chaineclef=="[TuxFisher]")
	{
		...
		{
			erreur = QObject::tr("La version du niveau n'est pas supportée.");
			return false;
		}
		...
		for (int c=0; c<nbcarte; c++)
		{
			...
			{
				erreur = QObject::tr("Les cartes du niveau n'ont pas pu être lu correctement.");
				return false;
			}
			donnes << octets;
		}
		...
		{
			erreur = QObject::tr("Les cartes lues ne correspondent pas au nombre de cartes présentes");
			return false;
		}
	}
	else
...
bool gestionNiveau_TuxFisher::changerNomNiveau(QString nom)
{
	...
	{
		nomniveau = nom;
		numeroversion=0; // Le fichier ne sera pas le même: remise a zéro de la version
		return true;
	}
	return false;
}

// Et si je change le nom du niveau, je sauvegarde, et je reviens a l'ancien nom du niveau? = (remise a zéro! BUG!)
